...
public class SummoningCircle {
    ...
    public SummoningCircle(Player player) {
        this.player = player;
    }
    
    public void evaluate() {
        try {
            if (isSummoningCircleFrozen(player)) {
                log.info("Summoning Circle is frozen");
                return;
            }
    ...
    private void internalEvaluate() {
        ...
        if (!FlagUtil.isActive(player, FlagScenario.FREEZE_SUMMONING_CIRCLE_ARTIFACT_FRAGMENT)) {
            collectArtifacts();
        }
    }

    private boolean isSummoningCircleFrozen(Player player) {
        return FlagUtil.isActive(player, FlagScenario.FREEZE_SUMMONING_CIRCLE_ARTIFACT_FRAGMENT)  &&
    ...
    private int getArtifactQuantity(BufferedImage input) {
        ...
        try {
            ...
            String numberAsText = OcrUtil.ocr(image, OcrUtil.WHITELIST_FOR_ONLY_NUMBERS, OcrUtil.PATTERN_FOR_ONLY_NUMBERS);
    ...
    private void collectCommonCaptainFragments(int qtd) {
        ...
        Point iconQuestionMarkPoint = ImageUtil.searchSurroundings(iconQuestionMark, screen, iconQuestionMarkArea, 0.15, 20).orElse(null);
    ...
    private void collectEliteCaptainFragments(int qtd) {
        ...
        if (qtd > 10) {
            robot.leftClick(iconQuestionMarkPoint.move(180, 56));
        }
        else {
            robot.leftClick(iconQuestionMarkPoint.move(0, 56));
        }
