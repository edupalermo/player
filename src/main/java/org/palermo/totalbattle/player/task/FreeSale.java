...
public class FreeSale {
    ...
    public void freeSale() {
        if (FlagUtil.isActive(player, FlagScenario.FREEZE_FREE_SALE_EVALUATION)) {
    ...
    private void internalFreeSale() {
        ...
        for (Point move: tabs) {
            ...
            Point buttonFreePoint = ImageUtil.search(buttonFree, screen, area, 0.05).orElse(null);
            ...
            if (iconHourglassPoint != null) {
                ...
                next = ImageUtil.toGrayscale(next, new String[] {"FFF7BF"});
